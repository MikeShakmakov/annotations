...
- **Нет персистентности** — `Save` только выводит JSON в консоль; данные не сохраняются в локал сторадж и теряются при перезагрузке.
...
|---|---|---|
| Аннотации не сохраняются | `save()` пишет в `console.log` | Добавить coхранение в локал сторадж в `AnnotationService` |
...
Чтобы открыть единственный доуступный документ нужно ввести адрес documents/1
Чтобы добавить или редактировать аннотацию используйте двойной клик
